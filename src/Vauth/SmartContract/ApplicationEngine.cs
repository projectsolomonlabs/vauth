...
using VMArray = Vauth.VM.Types.Array;

namespace Vauth.SmartContract
{
    /// <summary>
    /// A virtual machine used to execute smart contracts in the Vauth system.
    /// </summary>
    public partial class ApplicationEngine : ExecutionEngine
    {
        ...
        /// </summary>
        public const long TestModeValt = 20_00000000;
        ...
        /// <summary>
        /// Gets the descriptors of all interoperable services available in Vauth.
        /// </summary>
        ...
        /// <summary>
        /// The <see cref="Vauth.ProtocolSettings"/> used by the engine.
        /// </summary>
        ...
        /// </summary>
        public long ValtConsumed { get; private set; } = 0;

        /// <summary>
        /// The remaining VALT that can be spent in order to complete the execution.
        /// </summary>
        public long ValtLeft => valt_amount - ValtConsumed;
        ...
        /// <param name="settings">The <see cref="Vauth.ProtocolSettings"/> used by the engine.</param>
        /// <param name="valt">The maximum valt used in this execution. The execution will fail when the valt is exhausted.</param>
        protected ApplicationEngine(TriggerType trigger, IVerifiable container, DataCache snapshot, Block persistingBlock, ProtocolSettings settings, long valt)
        {
        ...
        /// <summary>
        /// Adds VALT to <see cref="ValtConsumed"/> and checks if it has exceeded the maximum limit.
        /// </summary>
        /// <param name="valt">The amount of VALT to be added.</param>
        protected internal void AddValt(long valt)
        {
            ValtConsumed = checked(ValtConsumed + valt);
            if (ValtConsumed > valt_amount)
                throw new InvalidOperationException("Insufficient VALT.");
        }

        internal void Refuel(long valt)
        {
            checked
            {
                valt_amount += valt;
        ...
        public static ApplicationEngine Create(TriggerType trigger, IVerifiable container, DataCache snapshot, Block persistingBlock = null, ProtocolSettings settings = null, long valt = TestModeValt)
        {
            return applicationEngineProvider?.Create(trigger, container, snapshot, persistingBlock, settings, valt)
                  ?? new ApplicationEngine(trigger, container, snapshot, persistingBlock, settings, valt);
        }
        ...
        protected virtual void OnSysCall(InteropDescriptor descriptor)
        {
            ...
            AddValt(descriptor.FixedPrice * exec_fee_factor);
        ...
        protected override void PreExecuteInstruction()
        {
            if (CurrentContext.InstructionPointer < CurrentContext.Script.Length)
                AddValt(exec_fee_factor * OpCodePrices[CurrentContext.CurrentInstruction.OpCode]);
        }
        ...
        /// <param name="valt">The maximum valt used in this execution. The execution will fail when the valt is exhausted.</param>
        /// <returns>The engine instance created.</returns>
        public static ApplicationEngine Run(byte[] script, DataCache snapshot, IVerifiable container = null, Block persistingBlock = null, ProtocolSettings settings = null, int offset = 0, long valt = TestModeValt)
        {
            persistingBlock ??= CreateDummyBlock(snapshot, settings ?? ProtocolSettings.Default);
            ApplicationEngine engine = Create(TriggerType.Application, container, snapshot, persistingBlock, settings, valt);
