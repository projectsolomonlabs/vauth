...
namespace Vauth.SmartContract
{
    ...
    {
        /// <summary>
        /// The maximum VALT that can be consumed when <see cref="VerifyWitnesses"/> is called.
        /// </summary>
        public const long MaxVerificationValt = 0_50000000;
        ...
        /// <param name="valt">The maximum VALT that can be used.</param>
        /// <returns><see langword="true"/> if the <see cref="IVerifiable"/> is verified as valid; otherwise, <see langword="false"/>.</returns>
        public static bool VerifyWitnesses(this IVerifiable verifiable, ProtocolSettings settings, DataCache snapshot, long valt)
        {
            if (valt < 0) return false;
            if (valt > MaxVerificationValt) valt = MaxVerificationValt;
            ...
            {
                if (!verifiable.VerifyWitness(settings, snapshot, hashes[i], verifiable.Witnesses[i], valt, out long fee))
                    return false;
                valt -= fee;
            }
            return true;
        ...
        internal static bool VerifyWitness(this IVerifiable verifiable, ProtocolSettings settings, DataCache snapshot, UInt160 hash, Witness witness, long valt, out long fee)
        {
            fee = 0;
            ...
            using (ApplicationEngine engine = ApplicationEngine.Create(TriggerType.Verification, verifiable, snapshot?.CreateSnapshot(), null, settings, valt))
            {
