...
{
    /// <summary>
    /// Represents the Vauth token in the Vauth system.
    /// </summary>
    public sealed class VauthToken : FungibleToken<VauthToken.VauthAccountState>
    {
        public override string Symbol => "Vauth";
        public override byte Decimals => 0;

        /// <summary>
        /// Indicates the total amount of Vauth.
        /// </summary>
        public BigInteger TotalAmount { get; }

        /// <summary>
        /// Indicates the effective voting turnout in Vauth. The voted candidates will only be effective when the voting turnout exceeds this value.
        /// </summary>
        ...
        private BigInteger CalculateBonus(DataCache snapshot, ECPoint vote, BigInteger value, uint start, uint end)
        {
            ...
            BigInteger VauthHolderReward = CalculateVauthHolderReward(snapshot, value, start, end);
            if (vote is null) return VauthHolderReward;
            ...
            BigInteger startRewardPerVauth = item ?? BigInteger.Zero;
            ...
            BigInteger endRewardPerVauth = item ?? BigInteger.Zero;

            return VauthHolderReward + value * (endRewardPerVauth - startRewardPerVauth) / 100000000L;
        }

        private BigInteger CalculateVauthHolderReward(DataCache snapshot, BigInteger value, uint start, uint end)
        {
            BigInteger sum = 0;
            foreach (var (index, valtPerBlock) in GetSortedValtRecords(snapshot, end - 1))
            {
                if (index > start)
                {
                    sum += valtPerBlock * (end - index);
                    end = index;
                }
                else
                {
                    sum += valtPerBlock * (end - start);
                    break;
                }
            }
            return value * sum * VauthHolderRewardRatio / 100 / TotalAmount;
        }
        ...
        internal override ContractTask Initialize(ApplicationEngine engine)
        {
            ...
            engine.Snapshot.Add(CreateStorageKey(Prefix_ValtPerBlock).AddBigEndian(0u), new StorageItem(5 * VALT.Factor));
            engine.Snapshot.Add(CreateStorageKey(Prefix_RegisterPrice), new StorageItem(1000 * VALT.Factor));
        ...
        internal override async ContractTask PostPersist(ApplicationEngine engine)
        {
            ...
            var valtPerBlock = GetValtPerBlock(engine.Snapshot);
            ...
            await VALT.Mint(engine, account, valtPerBlock * CommitteeRewardRatio / 100, false);

            // Record the cumulative reward of the voters of committee

            if (ShouldRefreshCommittee(engine.PersistingBlock.Index, m))
            {
                BigInteger voterRewardOfEachCommittee = valtPerBlock * VoterRewardRatio * 100000000L * m / (m + n) / 100; // Zoom in 100000000 times, and the final calculation should be divided 100000000L
                for (index = 0; index < committee.Count; index++)
                {
                    ...
                    if (member.Votes > 0)
                    {
                        BigInteger voterSumRewardPerVauth = factor * voterRewardOfEachCommittee / member.Votes;
                        ...
                        voterSumRewardPerVauth += (item ?? BigInteger.Zero);
                        engine.Snapshot.Add(voterRewardKey, new StorageItem(voterSumRewardPerVauth));
                    }
                }
            }
        }
        ...
        private void SetValtPerBlock(ApplicationEngine engine, BigInteger valtPerBlock)
        {
            if (valtPerBlock < 0 || valtPerBlock > 10 * VALT.Factor)
                throw new ArgumentOutOfRangeException(nameof(valtPerBlock));
            if (!CheckCommittee(engine)) throw new InvalidOperationException();

            uint index = engine.PersistingBlock.Index + 1;
            StorageItem entry = engine.Snapshot.GetAndChange(CreateStorageKey(Prefix_ValtPerBlock).AddBigEndian(index), () => new StorageItem(valtPerBlock));
            entry.Set(valtPerBlock);
        }

        /// <summary>
        /// Gets the amount of VALT generated in each block.
        /// </summary>
        /// <param name="snapshot">The snapshot used to read data.</param>
        /// <returns>The amount of VALT generated.</returns>
        [ContractMethod(CpuFee = 1 << 15, RequiredCallFlags = CallFlags.ReadStates)]
        public BigInteger GetValtPerBlock(DataCache snapshot)
        {
            return GetSortedValtRecords(snapshot, Ledger.CurrentIndex(snapshot) + 1).First().ValtPerBlock;
        }
        ...
        private IEnumerable<(uint Index, BigInteger ValtPerBlock)> GetSortedValtRecords(DataCache snapshot, uint end)
        {
            byte[] key = CreateStorageKey(Prefix_ValtPerBlock).AddBigEndian(end).ToArray();
            byte[] boundary = CreateStorageKey(Prefix_ValtPerBlock).ToArray();
        ...
        /// <summary>
        /// Get the amount of unclaimed VALT in the specified account.
        /// </summary>
        /// <param name="snapshot">The snapshot used to read data.</param>
        /// <param name="account">The account to check.</param>
        /// <param name="end">The block index used when calculating VALT.</param>
        /// <returns>The amount of unclaimed VALT.</returns>
        ...
        private bool RegisterCandidate(ApplicationEngine engine, ECPoint pubkey)
        {
            if (!engine.CheckWitnessInternal(Contract.CreateSignatureRedeemScript(pubkey).ToScriptHash()))
                return false;
            engine.AddValt(GetRegisterPrice(engine.Snapshot));
        ...
        /// <summary>
        /// Represents the account state of <see cref="VauthToken"/>.
        /// </summary>
        public class VauthAccountState : AccountState
        {
            /// <summary>
