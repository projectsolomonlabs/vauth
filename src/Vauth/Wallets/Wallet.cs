...
using System.Text;
using static Vauth.SmartContract.Helper;
using static Vauth.Wallets.Helper;
using ECPoint = Vauth.Cryptography.ECC.ECPoint;
...
namespace Vauth.Wallets
{
    ...
    {
        /// <summary>
        /// The <see cref="Vauth.ProtocolSettings"/> to be used by the wallet.
        /// </summary>
        ...
        /// <param name="settings">The <see cref="Vauth.ProtocolSettings"/> to be used by the wallet.</param>
        ...
        public BigDecimal GetBalance(DataCache snapshot, UInt160 asset_id, params UInt160[] accounts)
        {
            ...
            using ApplicationEngine engine = ApplicationEngine.Run(script, snapshot, settings: ProtocolSettings, valt: 20000000L * accounts.Length);
        ...
        /// <param name="version">The address version of Vauth system.</param>
        ...
        public Transaction MakeTransaction(DataCache snapshot, TransferOutput[] outputs, UInt160 from = null, Signer[] cosigners = null)
        {
            ...
            byte[] script;
            List<(UInt160 Account, BigInteger Value)> balances_valt = null;
            using (ScriptBuilder sb = new())
            {
                foreach (var (assetId, group, sum) in outputs.GroupBy(p => p.AssetId, (k, g) => (k, g, g.Select(p => p.Value.Value).Sum())))
                {
                    ...
                    if (assetId.Equals(NativeContract.VALT.Hash))
                        balances_valt = balances;
                }
                script = sb.ToArray();
            }
            if (balances_valt is null)
                balances_valt = accounts.Select(p => (Account: p, Value: NativeContract.VALT.BalanceOf(snapshot, p))).Where(p => p.Value.Sign > 0).ToList();

            return MakeTransaction(snapshot, script, cosignerList.Values.ToArray(), Array.Empty<TransactionAttribute>(), balances_valt);
        }
        ...
        /// <param name="maxValt">The maximum valt that can be spent to execute the script.</param>
        /// <returns>The created transction.</returns>
        public Transaction MakeTransaction(DataCache snapshot, byte[] script, UInt160 sender = null, Signer[] cosigners = null, TransactionAttribute[] attributes = null, long maxValt = ApplicationEngine.TestModeValt)
        {
            UInt160[] accounts;
            if (sender is null)
            {
            ...
            var balances_valt = accounts.Select(p => (Account: p, Value: NativeContract.VALT.BalanceOf(snapshot, p))).Where(p => p.Value.Sign > 0).ToList();
            return MakeTransaction(snapshot, script, cosigners ?? Array.Empty<Signer>(), attributes ?? Array.Empty<TransactionAttribute>(), balances_valt, maxValt);
        }

        private Transaction MakeTransaction(DataCache snapshot, byte[] script, Signer[] cosigners, TransactionAttribute[] attributes, List<(UInt160 Account, BigInteger Value)> balances_valt, long maxValt = ApplicationEngine.TestModeValt)
        {
            Random rand = new();
            foreach (var (account, value) in balances_valt)
            {
                Transaction tx = new()
                {
                    Version = 0,
                ...
                using (ApplicationEngine engine = ApplicationEngine.Run(script, snapshot.CreateSnapshot(), tx, settings: ProtocolSettings, valt: maxValt))
                {
                    if (engine.State == VMState.FAULT)
                    {
                        throw new InvalidOperationException($"Failed execution for '{Convert.ToBase64String(script)}'", engine.FaultException);
                    }
                    tx.SystemFee = engine.ValtConsumed;
                }

                tx.NetworkFee = CalculateNetworkFee(snapshot, tx);
                if (value >= tx.SystemFee + tx.NetworkFee) return tx;
            }
            throw new InvalidOperationException("Insufficient VALT");
        }
        ...
        public long CalculateNetworkFee(DataCache snapshot, Transaction tx)
        {
            ...
            foreach (UInt160 hash in hashes)
            {
                ...
                if (witness_script is null)
                {
                    ...
                    networkFee += engine.ValtConsumed;
                }
