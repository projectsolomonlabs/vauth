...
namespace Vauth.UnitTests.Cryptography
{
    ...
    public class UT_Crypto
    {
        ...
        public void TestVerifySignature()
        {
            ...
            wrongKey[0] = 0x02;
            Crypto.VerifySignature(message, signature, wrongKey, Vauth.Cryptography.ECC.ECCurve.Secp256r1).Should().BeFalse();

            wrongKey[0] = 0x03;
            for (int i = 1; i < 33; i++) wrongKey[i] = byte.MaxValue;
            Action action = () => Crypto.VerifySignature(message, signature, wrongKey, Vauth.Cryptography.ECC.ECCurve.Secp256r1);
            action.Should().Throw<ArgumentException>();

            wrongKey = new byte[36];
            action = () => Crypto.VerifySignature(message, signature, wrongKey, Vauth.Cryptography.ECC.ECCurve.Secp256r1);
        ...
        public void TestSecp256k1()
        {
            ...
            Crypto.VerifySignature(message, signature, pubKey, Vauth.Cryptography.ECC.ECCurve.Secp256k1)
                .Should().BeTrue();
