...
namespace Vauth.UnitTests.Ledger
{
    ...
    public class UT_MemoryPool : TestKit
    {
        ...
        public void TestSetup()
        {
            ...
            _unit = new MemoryPool(new VauthSystem(ProtocolSettings.Default with { MemoryPoolMaxTransactions = 100 }));
            ...
            _unit2 = new MemoryPool(new VauthSystem(ProtocolSettings.Default with { MemoryPoolMaxTransactions = 0 }));
        ...
        {
            var snapshot = GetSnapshot();
            BigInteger balance = NativeContract.VALT.BalanceOf(snapshot, senderAccount);
            ...
            _ = NativeContract.VALT.Mint(engine, UInt160.Zero, 70, true);
            ...
            ApplicationEngine applicationEngine = ApplicationEngine.Create(TriggerType.All, block, snapshot, block, settings: TestBlockchain.TheVauthSystem.Settings, valt: (long)balance);
            await NativeContract.VALT.Burn(applicationEngine, sender, NativeContract.VALT.BalanceOf(snapshot, sender));
            _ = NativeContract.VALT.Mint(applicationEngine, sender, txFee * 30, true); // Set the balance to meet 30 txs only
            ...
            // Revert the balance
            await NativeContract.VALT.Burn(applicationEngine, sender, txFee * 30);
            _ = NativeContract.VALT.Mint(applicationEngine, sender, balance, true);
        }
        ...
        {
            _unit = new MemoryPool(new VauthSystem(ProtocolSettings.Default with { MemoryPoolMaxTransactions = 600 }));
