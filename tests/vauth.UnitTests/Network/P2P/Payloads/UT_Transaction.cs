...
namespace Vauth.UnitTests.Network.P2P.Payloads
{
    ...
    public class UT_Transaction
    {
        ...
        [TestMethod]
        public void Valt_Get()
        {
            uut.SystemFee.Should().Be(0);
        }

        [TestMethod]
        public void Valt_Set()
        {
            long val = 4200000000;
            uut.SystemFee = val;
        ...
        public void FeeIsMultiSigContract()
        {
            ...
            var key = NativeContract.VALT.CreateStorageKey(20, acc.ScriptHash);
            var entry = snapshot.GetAndChange(key, () => new StorageItem(new AccountState()));

            entry.GetInteroperable<AccountState>().Balance = 10000 * NativeContract.VALT.Factor;
            ...
            long verificationValt = 0;
            foreach (var witness in tx.Witnesses)
            {
                using ApplicationEngine engine = ApplicationEngine.Create(TriggerType.Verification, tx, snapshot, settings: TestBlockchain.TheVauthSystem.Settings, valt: tx.NetworkFee);
                ...
                verificationValt += engine.ValtConsumed;
            }

            var sizeValt = tx.Size * NativeContract.Policy.GetFeePerByte(snapshot);
            Assert.AreEqual(1967100, verificationValt);
            Assert.AreEqual(348000, sizeValt);
        ...
        public void FeeIsSignatureContractDetailed()
        {
            ...
            Assert.AreEqual(245000, sizeValt);
            Assert.AreEqual(983520, verificationValt);

            // final assert
            Assert.AreEqual(tx.NetworkFee, verificationValt + sizeValt);
        }
        ...
        public void FeeIsSignatureContract_TestScope_Global()
        {
            ...
            using (ScriptBuilder sb = new())
            {
                ...
                sb.EmitDynamicCall(NativeContract.VALT.Hash, "transfer", acc.ScriptHash, acc.ScriptHash, value, null);
            ...
            Assert.AreEqual(1228520, verificationValt + sizeValt);
            // final assert
            Assert.AreEqual(tx.NetworkFee, verificationValt + sizeValt);
        }

        [TestMethod]
        public void FeeIsSignatureContract_TestScope_CurrentHash_VALT()
        {
            var wallet = TestUtils.GenerateTestWallet();
            var snapshot = TestBlockchain.GetTestSnapshot();
            ...
            var signers = new Signer[]{ new Signer
                {
                    ...
                    AllowedContracts = new[] { NativeContract.VALT.Hash }
                } };
            ...
            var sizeValt = tx.Size * NativeContract.Policy.GetFeePerByte(snapshot);
            // final check on sum: verification_cost + tx_size
            Assert.AreEqual(1249520, verificationValt + sizeValt);
            // final assert
            Assert.AreEqual(tx.NetworkFee, verificationValt + sizeValt);
        }

        [TestMethod]
        public void FeeIsSignatureContract_TestScope_CalledByEntry_Plus_VALT()
        {
            var wallet = TestUtils.GenerateTestWallet();
            var snapshot = TestBlockchain.GetTestSnapshot();
            ...
            // trying CalledByEntry together with VALT
            var signers = new Signer[]{ new Signer
                {
                    Account = acc.ScriptHash,
        ...
        [TestMethod]
        public void FeeIsSignatureContract_TestScope_CurrentHash_Vauth_FAULT()
        {
            var wallet = TestUtils.GenerateTestWallet();
            var snapshot = TestBlockchain.GetTestSnapshot();
            ...
            using (ScriptBuilder sb = new())
            {
                // self-transfer of 1e-8 VALT
                BigInteger value = new BigDecimal(BigInteger.One, 8).Value;
                sb.EmitDynamicCall(NativeContract.VALT.Hash, "transfer", acc.ScriptHash, acc.ScriptHash, value);
                sb.Emit(OpCode.ASSERT);
                script = sb.ToArray();
            }
            ...
            var signers = new Signer[]{ new Signer
                {
                    Account = acc.ScriptHash,
                    Scopes = WitnessScope.CustomContracts,
                    AllowedContracts = new[] { NativeContract.Vauth.Hash }
                } };
        ...
        [TestMethod]
        public void FeeIsSignatureContract_TestScope_CurrentHash_Vauth_VALT()
        {
            var wallet = TestUtils.GenerateTestWallet();
            var snapshot = TestBlockchain.GetTestSnapshot();
            ...
            var signers = new Signer[]{ new Signer
                {
                    Account = acc.ScriptHash,
                    Scopes = WitnessScope.CustomContracts,
                    AllowedContracts = new[] { NativeContract.Vauth.Hash, NativeContract.VALT.Hash }
                } };
            ...
            var sizeValt = tx.Size * NativeContract.Policy.GetFeePerByte(snapshot);
            // final check on sum: verification_cost + tx_size
            Assert.AreEqual(1269520, verificationValt + sizeValt);
            // final assert
            Assert.AreEqual(tx.NetworkFee, verificationValt + sizeValt);
        }
        ...
        public void Transaction_Reverify_Hashes_Length_Unequal_To_Witnesses_Length()
        {
            ...
            Transaction txSimple = new()
            {
                Version = 0x00,
                Nonce = 0x01020304,
                SystemFee = (long)BigInteger.Pow(10, 8), // 1 VALT 
        ...
        public void Transaction_Serialize_Deserialize_Simple()
        {
            ...
            sTx.ToHexString().Should().Be(
                "00" + // version
                "04030201" + // nonce
                "00e1f50500000000" + // system fee (1 VALT)
            ...
            // try to deserialize
            Transaction tx2 = Vauth.IO.Helper.AsSerializable<Transaction>(sTx);

            tx2.Version.Should().Be(0x00);
            tx2.Nonce.Should().Be(0x01020304);
            tx2.Sender.Should().Be(UInt160.Zero);
            tx2.SystemFee.Should().Be(0x0000000005f5e100); // 1 VALT (long)BigInteger.Pow(10, 8)
        ...
        public void Transaction_Serialize_Deserialize_DistinctCosigners()
        {
            ...
            Assert.ThrowsException<FormatException>(() =>
                tx2 = Vauth.IO.Helper.AsSerializable<Transaction>(sTx)
            );
            Assert.IsNull(tx2);
        }
        ...
        public void Transaction_Serialize_Deserialize_MaxSizeCosigners()
        {
            ...
            // back to transaction (should fail, due to non-distinct cosigners)
            Assert.ThrowsException<FormatException>(() => Vauth.IO.Helper.AsSerializable<Transaction>(sTx1));
            ...
            Transaction tx2 = null;
            Assert.ThrowsException<FormatException>(() =>
                tx2 = Vauth.IO.Helper.AsSerializable<Transaction>(sTx2)
            );
            Assert.IsNull(tx2);
        }
        ...
        public void Test_VerifyStateDependent()
        {
            ...
            var key = NativeContract.VALT.CreateStorageKey(20, tx.Sender);
            ...
            key = NativeContract.VALT.CreateStorageKey(20, acc.ScriptHash);
            balance = snapshot.GetAndChange(key, () => new StorageItem(new AccountState()));
            balance.GetInteroperable<AccountState>().Balance = 10000 * NativeContract.VALT.Factor;
