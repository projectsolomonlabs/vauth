...
using static Vauth.SmartContract.Native.VauthToken;

namespace Vauth.UnitTests.SmartContract.Native
{
    [TestClass]
    public class UT_VauthToken
    {
        ...
        [TestMethod]
        public void Check_Name() => NativeContract.Vauth.Name.Should().Be(nameof(VauthToken));

        [TestMethod]
        public void Check_Symbol() => NativeContract.Vauth.Symbol(_snapshot).Should().Be("Vauth");

        [TestMethod]
        public void Check_Decimals() => NativeContract.Vauth.Decimals(_snapshot).Should().Be(0);
        ...
        public void Check_Vote()
        {
            ...
            var accountState = snapshot.TryGet(CreateStorageKey(20, from)).GetInteroperable<VauthAccountState>();
        ...
        public void Check_Vote_Sameaccounts()
        {
            ...
            snapshot.Add(CreateStorageKey(20, G_Account), new StorageItem(new VauthAccountState { Balance = 200 }));
            var secondAccount = snapshot.TryGet(CreateStorageKey(20, G_Account)).GetInteroperable<VauthAccountState>();
        ...
        public void Check_Vote_ChangeVote()
        {
            ...
            snapshot.Add(CreateStorageKey(20, from_Account), new StorageItem(new VauthAccountState()));
            var accountState = snapshot.TryGet(CreateStorageKey(20, from_Account)).GetInteroperable<VauthAccountState>();
            accountState.Balance = 100;
            snapshot.Add(CreateStorageKey(33, ECCurve.Secp256r1.G.ToArray()), new StorageItem(new CandidateState()));
        ...
        [TestMethod]
        public void Check_UnclaimedValt()
        {
            var snapshot = _snapshot.CreateSnapshot();
            var persistingBlock = new Block { Header = new Header { Index = 1000 } };
            ...
            var unclaim = Check_UnclaimedValt(snapshot, from, persistingBlock);
            unclaim.Value.Should().Be(new BigInteger(0.5 * 1000 * 100000000L));
            unclaim.State.Should().BeTrue();

            unclaim = Check_UnclaimedValt(snapshot, new byte[19], persistingBlock);
        ...
        public void Check_RegisterValidator()
        {
            ...
            var members = NativeContract.Vauth.GetCandidates(snapshot);
        ...
        public void Check_UnregisterCandidate()
        {
            ...
            members = NativeContract.Vauth.GetCandidates(snapshot);
            ...
            ret.State.Should().BeTrue();
            var G_Account = Contract.CreateSignatureContract(ECCurve.Secp256r1.G).ScriptHash.ToArray();
            snapshot.Add(CreateStorageKey(20, G_Account), new StorageItem(new VauthAccountState()));
            var accountState = snapshot.TryGet(CreateStorageKey(20, G_Account)).GetInteroperable<VauthAccountState>();
            accountState.Balance = 100;
        ...
        public void Check_GetCommittee()
        {
            ...
            var committeemembers = NativeContract.Vauth.GetCommittee(snapshot);
            ...
            committeemembers = NativeContract.Vauth.GetCommittee(snapshot);
        ...
        public void Check_Transfer()
        {
            ...
            NativeContract.Vauth.Transfer(snapshot, from, to, BigInteger.One, false, persistingBlock).Should().BeFalse(); // Not signed
            NativeContract.Vauth.Transfer(snapshot, from, to, BigInteger.One, true, persistingBlock).Should().BeTrue();
            NativeContract.Vauth.BalanceOf(snapshot, from).Should().Be(99999999);
            NativeContract.Vauth.BalanceOf(snapshot, to).Should().Be(1);

            // Check unclaim

            unclaim = Check_UnclaimedValt(snapshot, from, persistingBlock);
            unclaim.Value.Should().Be(new BigInteger(0));
            unclaim.State.Should().BeTrue();

            snapshot.GetChangeSet().Count().Should().Be(keyCount + 4); // valt + new balance

            // Return balance

            keyCount = snapshot.GetChangeSet().Count();

            NativeContract.Vauth.Transfer(snapshot, to, from, BigInteger.One, true, persistingBlock).Should().BeTrue();
            NativeContract.Vauth.BalanceOf(snapshot, to).Should().Be(0);
            snapshot.GetChangeSet().Count().Should().Be(keyCount - 1);  // Remove Vauth balance from address two

            // Bad inputs

            NativeContract.Vauth.Transfer(snapshot, from, to, BigInteger.MinusOne, true, persistingBlock).Should().BeFalse();
            NativeContract.Vauth.Transfer(snapshot, new byte[19], to, BigInteger.One, false, persistingBlock).Should().BeFalse();
            NativeContract.Vauth.Transfer(snapshot, from, new byte[19], BigInteger.One, false, persistingBlock).Should().BeFalse();

            // More than balance

            NativeContract.Vauth.Transfer(snapshot, to, from, new BigInteger(2), true, persistingBlock).Should().BeFalse();
        }
        ...
        public void Check_BalanceOf()
        {
            ...
            NativeContract.Vauth.BalanceOf(snapshot, account).Should().Be(100_000_000);

            account[5]++; // Without existing balance

            NativeContract.Vauth.BalanceOf(snapshot, account).Should().Be(0);
        }
        ...
        public void Check_CommitteeBonus()
        {
            ...
            NativeContract.VALT.BalanceOf(snapshot, Contract.CreateSignatureContract(committee[0]).ScriptHash.ToArray()).Should().Be(50000000);
            NativeContract.VALT.BalanceOf(snapshot, Contract.CreateSignatureContract(committee[1]).ScriptHash.ToArray()).Should().Be(50000000);
            NativeContract.VALT.BalanceOf(snapshot, Contract.CreateSignatureContract(committee[2]).ScriptHash.ToArray()).Should().Be(0);
        }
        ...
        public void TestCalculateBonus()
        {
            ...
            snapshot.Add(key, new StorageItem(new VauthAccountState
            {
                Balance = -100
            }));
            Action action = () => NativeContract.Vauth.UnclaimedValt(snapshot, UInt160.Zero, 10).Should().Be(new BigInteger(0));
            action.Should().Throw<ArgumentOutOfRangeException>();
            snapshot.Delete(key);

            // Fault range: start >= end

            snapshot.GetAndChange(key, () => new StorageItem(new VauthAccountState
            {
                Balance = 100,
                BalanceHeight = 100
            }));
            action = () => NativeContract.Vauth.UnclaimedValt(snapshot, UInt160.Zero, 10).Should().Be(new BigInteger(0));
            snapshot.Delete(key);
            ...
            NativeContract.Vauth.UnclaimedValt(snapshot, UInt160.Zero, 100).Should().Be(new BigInteger(0.5 * 100 * 100));
            snapshot.Delete(key);
            ...
            snapshot.Add(new KeyBuilder(NativeContract.Vauth.Id, 23).Add(ProtocolSettings.Default.StandbyCommittee[0]).AddBigEndian(uint.MaxValue - 50), new StorageItem() { Value = new BigInteger(50 * 10000L).ToByteArray() });
            NativeContract.Vauth.UnclaimedValt(snapshot, UInt160.Zero, 100).Should().Be(new BigInteger(50 * 100));
            snapshot.Delete(key);
        }
        ...
        public void TestGetNextBlockValidators1()
        {
            ...
            var result = (VM.Types.Array)NativeContract.Vauth.Call(snapshot, "getNextBlockValidators");
        ...
        {
            var snapshot = _snapshot.CreateSnapshot();
            var result = NativeContract.Vauth.GetNextBlockValidators(snapshot, 7);
        ...
        {
            var snapshot = TestBlockchain.GetTestSnapshot();
            var array = (VM.Types.Array)NativeContract.Vauth.Call(snapshot, "getCandidates");
        ...
        {
            var snapshot = _snapshot.CreateSnapshot();
            var result = NativeContract.Vauth.GetCandidates(snapshot);
            result.Length.Should().Be(0);

            StorageKey key = NativeContract.Vauth.CreateStorageKey(33, ECCurve.Secp256r1.G);
            snapshot.Add(key, new StorageItem(new CandidateState()));
            NativeContract.Vauth.GetCandidates(snapshot).Length.Should().Be(1);
        }
        ...
        {
            var snapshot = _snapshot.CreateSnapshot();
            var committee = NativeContract.Vauth.GetCommittee(snapshot);
            var point = committee[0].EncodePoint(true);

            // Prepare Prefix_VoterRewardPerCommittee
            var storageKey = new KeyBuilder(NativeContract.Vauth.Id, 23).Add(committee[0]).AddBigEndian(20);
            snapshot.Add(storageKey, new StorageItem(new BigInteger(1000)));

            // Prepare Candidate
            storageKey = new KeyBuilder(NativeContract.Vauth.Id, 33).Add(committee[0]);
            snapshot.Add(storageKey, new StorageItem(new CandidateState { Registered = true, Votes = BigInteger.One }));

            storageKey = new KeyBuilder(NativeContract.Vauth.Id, 23).Add(committee[0]);
        ...
        {
            var snapshot = TestBlockchain.GetTestSnapshot();
            var result = (VM.Types.Array)NativeContract.Vauth.Call(snapshot, "getCommittee");
        ...
        {
            var snapshot = _snapshot.CreateSnapshot();
            var result = NativeContract.Vauth.ComputeNextBlockValidators(snapshot, ProtocolSettings.Default);
            result[0].ToArray().ToHexString().Should().Be("02486fd15702c4490a26703112a5cc1d0923fd697a33406bd5a1c00e0013b09a70");
            result[1].ToArray().ToHexString().Should().Be("024c7b7fb6c310fccf1ba33b082519d82964ea93868d676662d4a59ad548df0e7d");
            result[2].ToArray().ToHexString().Should().Be("02aaec38470f6aad0042c6e877cfd8087d2676b0f516fddd362801b9bd3936399e");
            result[3].ToArray().ToHexString().Should().Be("03b209fd4f53a7170ea4444e0cb0a6bb6a53c2bd016926989cf85f9b0fba17a70c");
        ...
        {
            var snapshot = _snapshot.CreateSnapshot();
            NativeContract.Vauth.TotalSupply(snapshot).Should().Be(new BigInteger(100000000));
        }
        ...
        public void TestEconomicParameter()
        {
            ...
            (BigInteger, bool) result = Check_GetValtPerBlock(snapshot, persistingBlock);
            result.Item2.Should().BeTrue();
            result.Item1.Should().Be(5 * NativeContract.VALT.Factor);

            persistingBlock = new Block { Header = new Header { Index = 10 } };
            (VM.Types.Boolean, bool) result1 = Check_SetValtPerBlock(snapshot, 10 * NativeContract.VALT.Factor, persistingBlock);
            ...
            result = Check_GetValtPerBlock(snapshot, persistingBlock);
            result.Item2.Should().BeTrue();
            result.Item1.Should().Be(10 * NativeContract.VALT.Factor);

            // Check calculate bonus
            StorageItem storage = snapshot.GetOrAdd(CreateStorageKey(20, UInt160.Zero.ToArray()), () => new StorageItem(new VauthAccountState()));
            VauthAccountState state = storage.GetInteroperable<VauthAccountState>();
            state.Balance = 1000;
            state.BalanceHeight = 0;
            height.Index = 0; // Fake Height=0
            NativeContract.Vauth.UnclaimedValt(snapshot, UInt160.Zero, persistingBlock.Index + 2).Should().Be(6500);
        }

        [TestMethod]
        public void TestClaimValt()
        {
            var snapshot = _snapshot.CreateSnapshot();
            ...
            for (var i = 0; i < ProtocolSettings.Default.CommitteeMembersCount; i++)
            {
                ...
                snapshot.Add(new KeyBuilder(NativeContract.Vauth.Id, 33).Add(member), new StorageItem(new CandidateState()
                {
                    Registered = true,
                    Votes = 200 * 10000
                }));
            ...
            snapshot.GetOrAdd(new KeyBuilder(NativeContract.Vauth.Id, 14), () => new StorageItem()).Value = BinarySerializer.Serialize(cachedCommittee.ToStackItem(null), 4096);

            var item = snapshot.GetAndChange(new KeyBuilder(NativeContract.Vauth.Id, 1), () => new StorageItem());
            ...
            NativeContract.Vauth.BalanceOf(snapshot, Contract.CreateSignatureContract(accountA).ScriptHash).Should().Be(0);

            StorageItem storageItem = snapshot.TryGet(new KeyBuilder(NativeContract.Vauth.Id, 23).Add(accountA).AddBigEndian(1));
            new BigInteger(storageItem.Value).Should().Be(30000000000);

            snapshot.TryGet(new KeyBuilder(NativeContract.Vauth.Id, 23).Add(accountB).AddBigEndian(uint.MaxValue - 1)).Should().BeNull();
            ...
            NativeContract.Vauth.BalanceOf(snapshot, Contract.CreateSignatureContract(committee[1]).ScriptHash).Should().Be(0);

            storageItem = snapshot.TryGet(new KeyBuilder(NativeContract.Vauth.Id, 23).Add(committee[1]).AddBigEndian(1));
            new BigInteger(storageItem.Value).Should().Be(30000000000);
            ...
            NativeContract.Vauth.BalanceOf(snapshot, Contract.CreateSignatureContract(committee[2]).ScriptHash).Should().Be(0);

            storageItem = snapshot.TryGet(new KeyBuilder(NativeContract.Vauth.Id, 23).Add(committee[2]).AddBigEndian(22));
            ...
            snapshot.Add(new KeyBuilder(NativeContract.Vauth.Id, 20).Add(account), new StorageItem(new VauthAccountState
            {
                BalanceHeight = 3,
                Balance = 200 * 10000 - 2 * 100,
                VoteTo = committee[2]
            }));
            NativeContract.Vauth.BalanceOf(snapshot, account).Should().Be(1999800);
            BigInteger value = NativeContract.Vauth.UnclaimedValt(snapshot, account, 29 + 3);
            value.Should().Be(1999800 * 30000000000 / 100000000L + (1999800L * 10 * 5 * 29 / 100));
        }

        [TestMethod]
        public void TestUnclaimedValt()
        {
            var snapshot = _snapshot.CreateSnapshot();
            NativeContract.Vauth.UnclaimedValt(snapshot, UInt160.Zero, 10).Should().Be(new BigInteger(0));
            snapshot.Add(CreateStorageKey(20, UInt160.Zero.ToArray()), new StorageItem(new VauthAccountState()));
            NativeContract.Vauth.UnclaimedValt(snapshot, UInt160.Zero, 10).Should().Be(new BigInteger(0));
        }
        ...
        public void TestVote()
        {
            ...
            snapshot.Add(keyAccount, new StorageItem(new VauthAccountState()));
            ret = Check_Vote(snapshot, account.ToArray(), ECCurve.Secp256r1.G.ToArray(), true, _persistingBlock);
            ret.State.Should().BeTrue();
            ret.Result.Should().BeFalse();

            snapshot.Delete(keyAccount);
            snapshot.GetAndChange(keyAccount, () => new StorageItem(new VauthAccountState
            {
                VoteTo = ECCurve.Secp256r1.G
            }));
        ...
        {
            var snapshot = _snapshot.CreateSnapshot();
            var engine = ApplicationEngine.Create(TriggerType.Application, TestBlockchain.TheVauthSystem.GenesisBlock, snapshot, TestBlockchain.TheVauthSystem.GenesisBlock, settings: TestBlockchain.TheVauthSystem.Settings);
            ...
            if (addVotes)
            {
                snapshot.Add(CreateStorageKey(20, from.ToArray()), new StorageItem(new VauthAccountState
                {
                    VoteTo = ECCurve.Secp256r1.G,
                    Balance = new BigInteger(1000)
                }));
                snapshot.Add(NativeContract.Vauth.CreateStorageKey(33, ECCurve.Secp256r1.G), new StorageItem(new CandidateState()));
            }
            else
            {
                snapshot.Add(CreateStorageKey(20, from.ToArray()), new StorageItem(new VauthAccountState
            ...
            sb.EmitDynamicCall(NativeContract.Vauth.Hash, "transfer", from, UInt160.Zero, amount, null);
        ...
        internal static bool Check_OnPersist(DataCache snapshot, Block persistingBlock)
        {
            ...
            var engine = ApplicationEngine.Create(TriggerType.OnPersist, null, snapshot, persistingBlock, settings: TestBlockchain.TheVauthSystem.Settings);
        ...
        internal static bool Check_PostPersist(DataCache snapshot, Block persistingBlock)
        {
            ...
            using var engine = ApplicationEngine.Create(TriggerType.PostPersist, null, snapshot, persistingBlock, settings: TestBlockchain.TheVauthSystem.Settings);
            engine.LoadScript(script.ToArray());

            return engine.Execute() == VMState.HALT;
        }

        internal static (BigInteger Value, bool State) Check_GetValtPerBlock(DataCache snapshot, Block persistingBlock)
        {
            using var engine = ApplicationEngine.Create(TriggerType.Application, null, snapshot, persistingBlock, settings: TestBlockchain.TheVauthSystem.Settings);

            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.Vauth.Hash, "getValtPerBlock");
            engine.LoadScript(script.ToArray());
        ...
        internal static (VM.Types.Boolean Value, bool State) Check_SetValtPerBlock(DataCache snapshot, BigInteger valtPerBlock, Block persistingBlock)
        {
            UInt160 committeeMultiSigAddr = NativeContract.Vauth.GetCommitteeAddress(snapshot);
            using var engine = ApplicationEngine.Create(TriggerType.Application, new Nep17NativeContractExtensions.ManualWitness(committeeMultiSigAddr), snapshot, persistingBlock, settings: TestBlockchain.TheVauthSystem.Settings);

            var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.Vauth.Hash, "setValtPerBlock", valtPerBlock);
            engine.LoadScript(script.ToArray());
        ...
        internal static (bool State, bool Result) Check_Vote(DataCache snapshot, byte[] account, byte[] pubkey, bool signAccount, Block persistingBlock)
        {
            using var engine = ApplicationEngine.Create(TriggerType.Application,
                new Nep17NativeContractExtensions.ManualWitness(signAccount ? new UInt160(account) : UInt160.Zero), snapshot, persistingBlock, settings: TestBlockchain.TheVauthSystem.Settings);

            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.Vauth.Hash, "vote", account, pubkey);
            engine.LoadScript(script.ToArray());
        ...
        {
            using var engine = ApplicationEngine.Create(TriggerType.Application,
                new Nep17NativeContractExtensions.ManualWitness(Contract.CreateSignatureRedeemScript(ECPoint.DecodePoint(pubkey, ECCurve.Secp256r1)).ToScriptHash()), snapshot, persistingBlock, settings: TestBlockchain.TheVauthSystem.Settings, valt: 1100_00000000);

            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.Vauth.Hash, "registerCandidate", pubkey);
            engine.LoadScript(script.ToArray());
        ...
        internal static ECPoint[] Check_GetCommittee(DataCache snapshot, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.Vauth.Hash, "getCommittee");
            engine.LoadScript(script.ToArray());
        ...
        internal static (BigInteger Value, bool State) Check_UnclaimedValt(DataCache snapshot, byte[] address, Block persistingBlock)
        {
            using var engine = ApplicationEngine.Create(TriggerType.Application, null, snapshot, persistingBlock, settings: TestBlockchain.TheVauthSystem.Settings);

            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.Vauth.Hash, "unclaimedValt", address, persistingBlock.Index);
            engine.LoadScript(script.ToArray());
        ...
        {
            using var engine = ApplicationEngine.Create(TriggerType.Application,
                new Nep17NativeContractExtensions.ManualWitness(Contract.CreateSignatureRedeemScript(ECPoint.DecodePoint(pubkey, ECCurve.Secp256r1)).ToScriptHash()), snapshot, persistingBlock, settings: TestBlockchain.TheVauthSystem.Settings);

            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.Vauth.Hash, "unregisterCandidate", pubkey);
            engine.LoadScript(script.ToArray());
