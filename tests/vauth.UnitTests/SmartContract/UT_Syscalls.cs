...
namespace Vauth.UnitTests.SmartContract
{
    ...
    public partial class UT_Syscalls : TestKit
    {
        ...
        [TestMethod]
        public void System_Runtime_ValtLeft()
        {
            var snapshot = TestBlockchain.GetTestSnapshot();
            ...
            using (var script = new ScriptBuilder())
            {
                ...
                script.EmitSysCall(ApplicationEngine.System_Runtime_ValtLeft);
                script.Emit(OpCode.NOP);
                script.EmitSysCall(ApplicationEngine.System_Runtime_ValtLeft);
                script.Emit(OpCode.NOP);
                script.Emit(OpCode.NOP);
                ...
                var engine = ApplicationEngine.Create(TriggerType.Application, null, snapshot, valt: 100_000_000);
                engine.LoadScript(script.ToArray());
                Assert.AreEqual(engine.Execute(), VMState.HALT);
