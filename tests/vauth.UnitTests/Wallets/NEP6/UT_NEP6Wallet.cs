...
using Contract = Vauth.SmartContract.Contract;

namespace Vauth.UnitTests.Wallets.NEP6
{
    [TestClass]
    public class UT_NEP6Wallet
    {
        ...
        {
            Vauth.SmartContract.Contract contract = Vauth.SmartContract.Contract.CreateSignatureContract(keyPair.PublicKey);
            bool result = uut.Contains(testScriptHash);
            Assert.AreEqual(false, result);
