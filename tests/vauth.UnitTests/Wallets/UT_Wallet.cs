...
using Vauth.UnitTests.Cryptography;
using Vauth.Wallets;
using System;
...
namespace Vauth.UnitTests.Wallets
{
    internal class MyWallet : Wallet
    {
        ...
        public override WalletAccount CreateAccount(byte[] privateKey)
        {
            ...
            Vauth.Wallets.SQLite.VerificationContract contract = new()
            {
    ...
    public class UT_Wallet
    {
        ...
        public void TestGetAvailable()
        {
            ...
            var key = NativeContract.VALT.CreateStorageKey(20, account.ScriptHash);
            var entry = snapshot.GetAndChange(key, () => new StorageItem(new AccountState()));
            entry.GetInteroperable<AccountState>().Balance = 10000 * NativeContract.VALT.Factor;
            snapshot.Commit();

            wallet.GetAvailable(snapshot, NativeContract.VALT.Hash).Should().Be(new BigDecimal(new BigInteger(1000000000000M), 8));
        ...
        public void TestGetBalance()
        {
            ...
            wallet.GetBalance(snapshot, NativeContract.VALT.Hash, new UInt160[] { account.ScriptHash }).Should().Be(new BigDecimal(new BigInteger(1000000000000M), 8));
        ...
        public void TestMakeTransaction1()
        {
            ...
            entry1.GetInteroperable<AccountState>().Balance = 10000 * NativeContract.VALT.Factor;

            key = NativeContract.Vauth.CreateStorageKey(20, account.ScriptHash);
            var entry2 = snapshot.GetAndChange(key, () => new StorageItem(new VauthToken.VauthAccountState()));
            entry2.GetInteroperable<VauthToken.VauthAccountState>().Balance = 10000 * NativeContract.Vauth.Factor;
            ...
            tx = wallet.MakeTransaction(snapshot, new TransferOutput[]
            {
                new TransferOutput()
                {
                     AssetId = NativeContract.Vauth.Hash,
                     ScriptHash = account.ScriptHash,
                     Value = new BigDecimal(BigInteger.One,8),
                     Data = "Dec 12th"
                }
            ...
            entry2.GetInteroperable<VauthToken.VauthAccountState>().Balance = 0;
            snapshot.Commit();
        }
        ...
        public void TestMakeTransaction2()
        {
            ...
            var key = NativeContract.VALT.CreateStorageKey(20, account.ScriptHash);
            var entry = snapshot.GetAndChange(key, () => new StorageItem(new AccountState()));
            entry.GetInteroperable<AccountState>().Balance = 1000000 * NativeContract.VALT.Factor;
            snapshot.Commit();
